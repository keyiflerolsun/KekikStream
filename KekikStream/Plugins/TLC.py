...
class TLC(PluginBase):
    name        = "TLC"
    language    = "tr"
    main_url    = "https://www.tlctv.com.tr"
    favicon     = f"https://www.google.com/s2/favicons?domain={main_url}&sz=64"
    description = "TLC canlı yayını, dizi ve programları yüksek kalite ve kesintisiz olarak izlemek için hemen tıkla! En sevdiğin TLC içerikleri burada."
    ...
    main_page = {
        ...
        f"{main_url}/kesfet/sira-disi-hayatlar" : "Sıra Dışı Hayatlar",
        f"{main_url}/kesfet/ev-dekorasyon"      : "Ev & Dekorasyon",
        f"{main_url}/kesfet/suc-arastirma"      : "Suç & Araştırma",
        f"{main_url}/kesfet/yasam"              : "Yaşam",
        f"{main_url}/kesfet/evlilik"            : "Evlilik",
        f"{main_url}/kesfet/yemek"              : "Yemek",
        f"{main_url}/kesfet/belgesel"           : "Belgesel",
        f"{main_url}/kesfet/korelendik"         : "Korelendik",
        f"{main_url}/kesfet/saglik-guzellik"    : "Sağlık & Güzellik",
    ...
    async def load_item(self, url: str) -> SeriesInfo:
        ...
        description = secici.select_text("div.slide-description p") or f"{title} programını TLC farkıyla izleyin."
        ...
        return SeriesInfo(
            ...
            year        = "2026",
            tags        = ["Yaşam", "Eğlence", "Belgesel"],
            actors      = ["TLC Sunucuları"],
            episodes    = episodes,
        )
    ...
    async def load_links(self, url: str) -> list[ExtractResult]:
        ...
        if not video_code:
            return []
        vid_url = f"https://dygvideo.dygdigital.com/api/redirect?PublisherId=20&ReferenceId={video_code}&SecretKey=NtvApiSecret2014*"
