...
from .            import konsol, cikis_yap, hata_yakala, RemotePlugin
from ..Core       import PluginManager, ExtractorManager, UIManager, MediaManager, PluginBase, ExtractorBase, SeriesInfo, ExtractResult
from .SearchUtils import normalize_search_text, calculate_similarity_score
import httpx, os
...
class KekikStream:
    ...
    async def start(self):
        ...
        if not self.plugin.get_plugin_names():
            konsol.print("[yellow]Lokal eklenti bulunamadı! Uzak API sunucuları aranıyor...[/yellow]")
            servers        = ["https://stream.watchbuddy.tv/api/v1", "https://example.watchbuddy.tv/api/v1"]
            server_options = []

            async with httpx.AsyncClient(timeout=3) as client:
                for srv in servers:
                    try:
                        res = await client.get(f"{srv}/schema")
            ...
            if cevap == "exit" or not cevap:
                return

            api_url = cevap
            if cevap == "custom":
                api_url = await self.ui.prompt_text("API Sunucu Adresi (Örn: https://sunucu.com/api/v1):")
                if not api_url:
                    return
                api_url = api_url.rstrip("/")

            try:
                konsol.print(f"[yellow]Sunucuya bağlanılıyor: {api_url}...[/yellow]")
                async with httpx.AsyncClient(timeout=10) as client:
                    resp = await client.get(f"{api_url}/get_all_plugins")
                    resp.raise_for_status()
                    plugins_data = resp.json().get("result", [])

                    for p_data in plugins_data:
                        p_name = p_data.get("name")
                        if p_name:
                            self.plugin.plugins[p_name] = RemotePlugin(name=p_name, base_api_url=api_url, plugin_data=p_data, ex_manager=self.extractor)

            except Exception as e:
                konsol.print(f"[bold red]Sunucu bağlantı hatası: {e}[/bold red]")
